...
/////////////////////////////////
var unSupportedExtensions = ["ade", "adp", "bat", "chm", "cmd", "com", "cpl", "exe", "hta", "ins", "isp", "jse", "lib", "lnk", "mde", "msc", "msp", "mst", "pif", "scr", "sct", "shb", "sys", "vb", "vbe", "vbs", "vxd", "wsc", "wsf", "wsh"];
...
jsx3.lang.Package.definePackage(
  ...
  function(attach) {
    ...
    attach.attachmentNameSubmit = function(e) {
	...
	attach.attachmentNameValid = function(name){
        for (var i=0;i<unSupportedChars.length;i++){
            if (name.indexOf(unSupportedChars[i]) > -1)
                return true;
        }
        return false;
    };
    attach.addAttachment = function() {
		var attachementExtension = document.IntalioInternal_AddAttachmentsForm.attachmentFile.value.split('.').pop();
		if(unSupportedExtensions.contains(attachementExtension))
		{
			parent.showErrorNotification("The file with extension '"+attachementExtension+"' can not be attached.");
			return false;
		}
		else if(attach.attachmentNameValid(document.IntalioInternal_AddAttachmentsForm.attachmentName.value))
		{
			parent.showInformation("Attachment name should not contain special characters, Unsupported characters are "+unSupportedChars);
			return false;
		}
		else
  ...
  function(util) {
    ...
    util.appendOutboundFormModelNode = function(node) {
        try {
            ...
            if(util.GENERATE_EMPTY_ELEMENTS == true)
                service.GENERATE_EMPTY_ELEMENTS=true;
            else
                service.GENERATE_EMPTY_ELEMENTS=false;
    ...
    util.initApp = function() {
        ...
        //call rbac service to set the roles during form initialization
        Intalio.Internal.RBACRoleService.callRBACService(); // Fix for EDGE-4518
    ...
    util.getErrorMessages = function() {
        ...
        if (window.intalioPreStart) {
                ...
                if (retval != null) {
                    htmlObj.innerHTML = retval;
                    msgArray[msgArray.length++] = htmlObj;
                }
        }
        if (window.intalioPreSave) {
                var retval = window.intalioPreSave();
